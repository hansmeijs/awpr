...
//let grade_rows = [];
const grade_dicts = {}; //PR2023-04-03 new approach, use dict of dicts instead of sorted list
//let subject_rows = [];
...
// --- get data stored in page
    ...
    mod_MCOL_dict.columns.btn_ep_01 = {exam_name: "Exam"};
    mod_MCOL_dict.columns.btn_reex = {exam_name: "Exam"};
    mod_MCOL_dict.columns.btn_reex03 = {exam_name: "Exam"};
...
// --- get field_settings
    ...
    field_settings.btn_ep_01 = {
        field_names: ["select", "examnumber", "fullname", "lvl_abbrev", "sct_abbrev", "cluster_name", "subj_code", "subj_name_nl",
                       ...
                       "note_status", "exam_name", "download_conv_table"],
...
// ---  SIDEBAR ------------------------------------
        ...
        if (el_SBR_select_subject){
             el_SBR_select_subject.addEventListener("click", function() {
                // PR204-03-28 was: t_MSSSS_Open(loc, "subject", subject_rows, add_all, false, setting_dict, permit_dict, MSSSS_subject_response)
                t_MSSSS_Open_NEW("sbr", "subject", subject_rows, MSSSS_subject_response, add_all)
            }, false);
        };

        const el_SBR_select_cluster = document.getElementById("id_SBR_select_cluster");
        if (el_SBR_select_cluster){
            el_SBR_select_cluster.addEventListener("click", function() {
                t_MSSSS_Open_NEW("sbr", "cluster", cluster_dictsNEW, MSSSS_cluster_response, add_all);
            }, false);
        };
        ...
        if (el_SBR_select_student){
            el_SBR_select_student.addEventListener("click", function() {
               // PR2024-05-02 was:  t_MSSSS_Open(loc, "student", student_rows, add_all, false, setting_dict, permit_dict, MSSSS_student_response)
                t_MSSSS_Open_NEW ("sbr", "student", student_rows, MSSSS_student_response, add_all);
            }, false);
        };
...
// ---  MOD APPROVE GRADE ------------------------------------
        ...
        if (el_MAG_auth_index){
            el_MAG_auth_index.addEventListener("click", function() {MAG_ChangeAuthMessage()}, false );
        };
...
// ---  MSELEX MOD SELECT EXAM ------------------------------
        ...
        const el_MSELEX_msg_container = document.getElementById("id_MSELEX_msg_container");
...
// ---  show loader
        ...
        console.log("permit_dict", permit_dict);
                ...
                if ("grade_rows" in response) {
                    //was: grade_rows = response.grade_rows;
                    b_fill_datadicts("grade", "id", null, response.grade_rows, grade_dicts);
...
//=========  SBR_lvl_sct_response  ================ PR2023-03-29 PR2023-06-08
...
// --- reset table rows, also delete header, item_count
        //tblHead_datatable.innerText = null;
        // happens in t_SBR_select_level_sector: tblBody_datatable.innerText = null;
...
// --- create table rows
        if(grade_dicts){
            // --- loop through grade_dicts
            for (const data_dict of Object.values(grade_dicts)) {
            ...
            // show only rows that has_practexam when sel_examtype = "pe"
                ...
                const show_row = //(!setting_dict.sel_lvlbase_pk || data_dict.lvlbase_id === setting_dict.sel_lvlbase_pk) &&
                                ...
                                // PR2024-03-29 deprecated, use sel_subjbase_pk instead
                                //(!setting_dict.sel_subject_pk || data_dict.subject_id === setting_dict.sel_subject_pk) &&
                                // PR2024-03-29 filtering takes place on server side, TODO change like in page studsubjects
                                //(!setting_dict.sel_subjbase_pk || data_dict.subject_id === setting_dict.sel_subject_pk) &&
...
// - filter tableRows when table is not empty
...
// - when table is empty and examperiod = 2 or 3: show msg 'Only the re-examination subjects will be shown'
...
// when user is corector and no grades showing: give msg 'You don't have any allowed subjects'
        } else if ( permit_dict.requsr_role_corr && selected_btn === "btn_ep_01") {
            if (!permit_dict.allowed_subjbases || !permit_dict.allowed_subjbases.length) {
                let tblRow = tblBody_datatable.insertRow(-1);
                let td = tblRow.insertCell(-1);
                td = tblRow.insertCell(-1);
                ...
                let el = document.createElement("p");
                el.className = "border_bg_transparent p-2 my-4"
                el.innerHTML =  loc.no_allowed_subj_msg_01 + "<br>" + loc.no_allowed_subj_msg_02+ "<br>" + loc.no_allowed_subj_msg_03;
                td.appendChild(el);
            };
        };
    };  // FillTblRows
...
// --- make el readonly when not requsr_requsr_same_school or when not edit permission
                    ...
                    if (may_edit){
                        ...
                        if (data_dict.examperiod === 4) {
                            // PR2024-06-28 TODO remove exemption_imported filter, too strict > maybe enable change when insp agrees
                            // was: is_enabled = ["segrade", "cegrade"].includes(field_name) && !data_dict.exemption_imported;
...
//=========  UpdateField  ================ PR2020-12-18 PR2021-05-30 PR2023-07-03 PR2024-06-29
    ...
    //console.log("     data_dict", data_dict);
            ...
            if(field_name){
                ...
                let strike_through = data_dict.studsubj_tobedeleted;
    ...
    // +++  create href and put it in button PR2021-05-07
                    const has_published_ce_exam = !!data_dict.ce_exam_id && !!data_dict.ce_exam_published_id;
                    const href_str = (has_published_ce_exam) ? urls.url_exam_download_conversion_pdf.replace("-", data_dict.ce_exam_id.toString()) : null;

                    el_div.innerHTML = (has_published_ce_exam) ? "&emsp;&#8681;&emsp;" : "&emsp;&emsp;&emsp;";
                    add_or_remove_attr(el_div, "href", has_published_ce_exam, href_str);
                    add_or_remove_attr(el_div, "target", has_published_ce_exam, "_blank");
                    add_or_remove_attr(el_div.parentNode, "title", has_published_ce_exam, loc.Download_conv_table);
                    add_or_remove_class(el_div.parentNode, "awp_pointer_show", has_published_ce_exam, "awp_pointer_hide" )
                    ...
                    //    inner_text = format_dateISO_vanilla (loc, data_dict.datepublished, true, false, true, false);
                        ...
                        //PR2024-06-29 hide exam name when not published. May happen when ETE cancel published
                            ...
                            //PR2024-07-03 debug: CVTE exams are not published, add "&& ete_exam"
                            if (!!data_dict.ce_exam_id && !data_dict.exam_published_id && data_dict.ete_exam) {
                                strike_through = true;
                                title_text += "\n" + ((!data_dict.ete_exam) ? loc.exam_publ_cancel_DES : loc.exam_publ_cancel_ETE);
                                title_text += "\n" + loc.exam_republished1;
                                title_text += "\n" + ((data_dict.secret_exam) ? loc.grade_willbe_calculated  : loc.score_canbe_entered);
                            };
                        };
                    };
...
//=========  UpdateFieldStatus  ================ PR2021-12-19 PR2022-08-28 PR2023-01-24
    function UpdateFieldStatus(field_name, fld_value, data_dict) {
        ...
        if (prefix_str === "note"){
            ...
            const is_secret_exam = (data_dict.secret_exam);
...
// --- set focus to next / previous cell
            ...
            // PR2023-04-21 Sentry error: Cannot read properties of null (reading 'previousElementSibling')
            // solved by adding if (new_tblRow.nextElementSibling) and if (new_tblRow.previousElementSibling){
            // tobe checked if it works ok
            ...
            if (move_vertical){
                while (!stop_loop && count < 100) {
                    ...
                    if (move_vertical > 0){
                        if (new_tblRow.nextElementSibling){
                            new_tblRow = new_tblRow.nextElementSibling;
                        } else {
                            stop_loop = true;
                        };
                    } else if (move_vertical < 0){
                        if (new_tblRow.previousElementSibling){
...
//========= HandleInputChange  ===============PR2020-08-16 PR2021-03-25 PR2021-09-20 PR2023-04-03
    function HandleInputChange(el_input){
        console.log(" --- HandleInputChange ---")

// ---  get selected.data_dict
        const tblRow = t_get_tablerow_selected(el_input)
        //PR2023-04-03 new approach, use dict of dicts instead of sorted list
        const map_id = tblRow.id
        const data_dict = grade_dicts[map_id];
        ...
        if (data_dict){
            ...
            let border_class = "";
            let msg_txt = null;
// --- check permission - must come after get datadict, to restore old value
...
// --- is_allowed_cluster = true when allowed_clusters is empty or when cluster_id in allowed_clusters PR2023-03-27
            ...
            if (!has_permit){
                ...
                msg_txt = loc.grade_err_list.no_permission;
    // - schools cannot enter score of secret exam
                ...
                const edit_txt = (examtype_2char === "ce" ) ? loc.grade_err_list.no_permission_edit_score : loc.grade_err_list.no_permission_edit_grade;
                msg_txt = [loc.grade_err_list.no_permission_cluster_01, edit_txt].join("<br>");
            };

            if (msg_txt) {
                const msg_html = ["<p class='p-2 ", border_class, "'>", msg_txt, "</p>"].join("");
...
console.log("is_blocked", is_blocked)
                ...
                if (is_submitted ){
                    ...
                    if (is_blocked){
                        b_show_mod_message_html(loc.grade_err_list.blocked_and_submitted01 + "<br>" + loc.grade_err_list.blocked_and_submitted02);
                       ...
                       //                               loc.grade_err_list.grade_approved + "<br>" + loc.grade_err_list.needs_approvals_removed+ "<br>" + loc.grade_err_list.Then_you_can_change_it;
                        ...
                        b_show_mod_message_html(loc.grade_err_list.grade_submitted + "<br>" + loc.grade_err_list.need_permission_inspection);
                    };
            ...
            // put back old value  in el_input
                    ...
                    if(new_value !== old_value){
                        // FOR TESTING ONLY: turn validate off to test server validation
                        // PR2023-04-03 let it turned off, tehre mauy be mistakes in ValidateGrade
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if ("grade_note_icon_rows" in response) {
                    ...
                    RefreshDataRows("grades", response.grade_note_icon_rows, grade_dicts, is_update, skip_show_ok);
                }
               if ("grade_stat_icon_rows" in response) {
                    const tblName = "grades", is_update = true;
                    RefreshDataRows(tblName, response.grade_stat_icon_rows, grade_dicts, is_update);
                }
            },
            error: function (xhr, msg) {
// ---  hide loader
...
//========= UploadStatus  ============= PR2020-07-31 PR2021-01-14 PR2022-03-20 PR2024-02-02
    function UploadStatus(el_input) {
        ...
        if(permit_dict.permit_approve_grade || permit_dict.permit_block_grade){
            ...
            const grade_pk = (data_dict) ? data_dict.id : null;
            console.log( "    data_dict", data_dict);
...
// - get info of this grade
                const examtype_2char = fldName.substring(0,2);
                const is_se_grades = examtype_2char === "se";
                ...
                // is_allowed_cluster = true when allowed_clusters is empty or when cluster_id in allowed_clusters PR2023-03-27
                // PR2023-05-28 debug. This doent work when user has only allowed_clusters for other schools
...
// +++ approve grades by school +++++++++++++++++++++
        ...
        // - schools cannot approve secret exam
                            if(data_dict.secret_exam && examtype_2char === "ce" && !permit_dict.requsr_role_admin){
        ...
        // - give message and exit when corrector wants to approve se_grade, not when remove approved
                                if (fldName === "se_status" && requsr_auth_index === 4 && !is_approved_by_requsr_auth){
                                    b_show_mod_message_html(loc.approve_err_list.Corrector_cannot_approve_se, loc.Approve_grades);
...
// ++++++++++++++++ Inspectorate can block grades +++++++++++++++++++++
    ...
    // check if cluster is allowed
                    ...
                    // must also be able to unlock when grade is (partly) approved

                const is_published = (!!data_dict[examtype_2char + "_published_id"]);
                    if (is_published || is_blocked
                        || !!data_dict[examtype_2char + "_auth1by_id"]
                        || !!data_dict[examtype_2char + "_auth2by_id"]
                        || !!data_dict[examtype_2char + "_auth3by_id"]
                        || !!data_dict[examtype_2char + "_auth4by_id"]
                        || true
                    ){
                        if (!is_allowed_cluster){
                            const msg_txt = (!is_blocked) ? loc.No_cluster_unlock_permission : loc.No_cluster_remove_unlocking_permission;
                            ...
                            UploadStatus_get_locked_unlocked_rows();

                            ModConfirmOpen("unlock_grade")
                        };
                    };  //  if (is_published || is_blocked)
                };  // if(permit_dict.permit_approve_grade)
            };  //   if(!isEmpty(data_dict)){
...
//=========  UploadStatus_get_locked_unlocked_rows  ================ PR2023-04-08
    function UploadStatus_get_locked_unlocked_rows() {
        console.log(" --- UploadStatus_get_locked_unlocked_rows --- ");

        const is_se_grades = mod_dict.examtype_2char === "se";
        const sel_mapid =  mod_dict.data_dict.mapid;
        const sel_tr_is_blocked = mod_dict.is_blocked;

// ---  loop through tblBody.rows and fill grade_pk_count
        // select on se_blocked or ce_blocked, is_blocked or not
        // always include tr_clicked, because it has not yet cls_selected when clicked on icon
        ...
        for (let i = 0, tblRow; tblRow = tblBody_datatable.rows[i]; i++) {
            ...
            if (tblRow.classList.contains(cls_selected) || tblRow.id === sel_mapid) {
                const data_dict = grade_dicts[tblRow.id];
                if (data_dict){
                    const grade_is_blocked = (is_se_grades) ? data_dict.se_blocked  : data_dict.ce_blocked;
                    const add_to_list = (sel_tr_is_blocked) ? grade_is_blocked : !grade_is_blocked;
                    if (add_to_list){
                        mod_dict.grade_pk_list.push(data_dict.id);
                    };
                };
            };
        };
        mod_dict.grade_pk_count = (mod_dict.grade_pk_list) ? mod_dict.grade_pk_list.length : 0;
    };  // UploadStatus_get_locked_unlocked_rows
...
//========= UploadApproveGrade  ============= PR2022-03-12 PR2023-02-02
    function UploadApproveGrade(tblName, fldName, examtype_2char, data_dict, auth_dict, requsr_auth_index,
        ...
        console.log( "    data_dict", data_dict);
        // called by UploadStatus and  ModConfirmSave when mode === "approve"
...
//========= UploadBlockGrade  ============= PR2022-04-16 PR2023-04-08
    function UploadBlockGrade() {
        console.log( " ==== UploadBlockGrade ====");
        console.log("    grade_pk_list: ", mod_dict.grade_pk_list);
...
// ---  change icon, before uploading
        ...
        if (!is_allowed_cluster){
            const msg_txt = (new_is_blocked) ? loc.No_cluster_unlock_permission : loc.No_cluster_remove_unlocking_permission ;
            const msg_html = ["<div class='p-2'>", msg_txt, "</div>"].join("");
            b_show_mod_message_html(msg_html);
        } else {
    ...
    // ---  upload changes
            ...
            const mode = (new_blocked) ? "block" : "unblock"
            const upload_dict = { mode: mode,
                                  grade_pk_list: mod_dict.grade_pk_list,
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if ("updated_grade_rows" in response) {
                        RefreshDataRows("grades", response.updated_grade_rows, grade_dicts, true); // true = is_update
                    };
...
//=========  ModConfirmOpen  ================ PR2020-08-03 PR2022-02-17 PR2022-04-17
    function ModConfirmOpen(mode, el_input) {
        ...
        // values of mode are : "approve", "unlock_grade", "prelim_ex2", "prelim_ex2a"
        ...
        // unlock_grade is called when clicked on status icon of tblRow
        } else if (mode === "unlock_grade"){
            //el_confirm_header.innerText = (mod_dict.is_blocked) ? loc.Remove_unlocking_grades : loc.Unlock_grades;

            el_confirm_header.innerText = (mod_dict.is_se_grades) ? (mod_dict.is_blocked) ? loc.Remove_unlocking_se_grades : loc.Unlock_se_grades :
                                                       (mod_dict.is_blocked) ? loc.Remove_unlocking_scores : loc.Unlock_scores;
            const count_txt = (mod_dict.is_se_grades) ? ( mod_dict.grade_pk_count === 1) ? loc.SE_grade : loc.SE_grades :
                             ( mod_dict.grade_pk_count === 1) ? loc.CE_score : loc.CE_scores;

            const msg_list = [];
            if (mod_dict.is_blocked) {
                msg_list.push(...["<p>",
                    loc.MAG_info.remove_unlock_multiple_01,  // "You are about remove the unlocking of "
                    mod_dict.grade_pk_count, " ", count_txt, loc.MAG_info.remove_unlock_multiple_02, ". ",
                    "</p><p class='pt-2'>", loc.Do_you_want_to_continue, "</p>"
                ]);
            } else {
                // see https://stackoverflow.com/questions/4842993/javascript-push-an-entire-list
                msg_list.push(...["<p>",
                    loc.MAG_info.unlock_multiple_01,  // You are about to unlock
                    mod_dict.grade_pk_count, " ", count_txt, " ",
                    loc.MAG_info.unlock_multiple_02,
                    "</p><p class='p-2'><small>",
                        loc.MAG_info.unlock_02, " ",
                        loc.MAG_info.unlock_03, " ",
                        loc.MAG_info.unlock_04,
                    "</small></p>",
                    //"<p>",
                       // loc.MAG_info.unlock_05, " ",  // "Please add a note with an explanation and include the grade in the note."
                       // loc.MAG_info.unlock_06,  // "After unlocking the grade, the value of the grade will no longer be visible for you."
                    //"</p>",
                    "<p class='pt-2'>", loc.Do_you_want_to_continue, "</p>"
                ]);
            };
            ...
            el_confirm_btn_save.innerText = (mod_dict.is_blocked) ? loc.Yes_remove : loc.Yes_unlock;
        ...
        // show modal
            ...
            add_or_remove_class(document.getElementById("id_mod_confirm_size"), "modal-lg", !mod_dict.is_blocked, "modal-md");
...
//=========  ModConfirmSave  ================ PR2019-06-23 PR2023-04-08
    function ModConfirmSave() {
        ...
        } else if (mod_dict.mode === "unlock_grade"){
...
// --- check sel_examperiod
        ...
        mod_MAG_dict.is_examperiod_exemption = (mod_MAG_dict.examperiod === 4);
...
// get selected auth_index (user can be chairpeson / secretary and examiner at the same time)
            ...
            if (setting_dict.sel_auth_index === 4 && mod_MAG_dict.is_examperiod_exemption &&  !["pe", "ce"].includes(setting_dict.sel_examtype) ){
    ...
    // --- hide select subject and cluster when submit mode
                    ...
                    add_or_remove_class(el_MAG_cluster.parentNode, cls_hide, !mod_MAG_dict.is_approve_mode || mod_MAG_dict.is_examperiod_exemption);
    // - hide lvlbase when Havo Vwo PR2024-05-02
...
//=========  MAG_fill_select_authindex  ================
    function MAG_fill_select_authindex () {  // PR2023-02-06 PR2024-06-14
    ...
    // --- fill selectbox auth_index
        if (el_MAG_auth_index){
            ...
            const auth_caption = (setting_dict.sel_auth_index) ? cpt_list[setting_dict.sel_auth_index] : "---";
            el_MAG_auth_index.innerText = auth_caption;
        };
    };  // MAG_fill_select_authindex
...
//=========  MAG_ChangeAuthMessage  ================ PR2024-06-14
    function MAG_ChangeAuthMessage () {
        //console.log("===  MAG_ChangeAuthMessage  =====") ;

        const msg_html = ["<p class='p-2'>",
         loc.MAG_info.msg_change_auth01, "<br>",
         loc.MAG_info.msg_change_auth02,
         "</p>"].join("");
        b_show_mod_message_html(msg_html);


    }; // MAG_ChangeAuthMessage

    function MAG_set_cluster_txt() { // PR2024-05-02
        //console.log("----- MAG_set_cluster_txt -----") ;
        //console.log("    setting_dict", setting_dict) ;
        //console.log("    permit_dict", permit_dict) ;
       // console.log("    cluster_dictsNEW", cluster_dictsNEW) ;
        ...
        let cluster_txt = "", title_txt = "";
        let count_clusters = 0

// - leave text blank when the school has no clusters
        if (!isEmpty(cluster_dictsNEW)) {
            ...
            // therefore: don't filter on allowed subjects / clusters when selected auth_index is chairperson or secretary
...
// - check if cluster is in allowed subjbases  (skip when auth1 auth2)
                if (user_is_auth1_or_auth2 ||
                    ...
                    permit_dict.allowed_subjbases.includes(cluster_dict.subjbase_id)) {

        //console.log("    cluster_dict", cluster_dict) ;
        //console.log("    cluster is in allowed subjbases") ;
// - check if cluster_subjbase is selected subjbase (or no selected subjbase)
                    if (!setting_dict.sel_subjbase_pk || setting_dict.sel_subjbase_pk === cluster_dict.subjbase_id){
                        count_clusters += 1;
        //console.log("    cluster_subjbase is selected subjbase (or no selected subjbase") ;
// - check if cluster is in allowed clusters (skip when auth1 auth2)
                        if (user_is_auth1_or_auth2 ||
                            !permit_dict.allowed_clusters ||
                            !permit_dict.allowed_clusters.length ||
                            permit_dict.allowed_clusters.includes(cluster_dict.id)) {

        //console.log("    cluster is in allowed clusters") ;
// - check if cluster is selected (or there is no selected cluster)
                            if (!setting_dict.sel_cluster_pk || setting_dict.sel_cluster_pk === cluster_dict.id) {
        //console.log("    cluster is selected (or there is no selected cluster") ;
                                allowed_clustername_arr.push(cluster_dict.name);
                            };
                        };
                    };
                };
    ...
    //console.log("    count_clusters", count_clusters) ;
    //console.log("    allowed_clustername_arr", allowed_clustername_arr) ;
                //cluster_txt = loc.All_clusters;
        };

        if (allowed_clustername_arr.length) {
            if (allowed_clustername_arr.length === count_clusters){
                cluster_txt = loc.All_clusters;
            } else {
                allowed_clustername_arr.sort();
                cluster_txt = allowed_clustername_arr.join(", ");
                title_txt = allowed_clustername_arr.join("\n");
            };
        } else {
            cluster_txt = loc.All_clusters;
        };

        el_MAG_cluster.innerText = cluster_txt;
        el_MAG_cluster.title = title_txt;
    }; // MAG_set_cluster_txt
    ...
    function MAG_set_subject_txt() { // PR2023-02-11 PR2024-05-02
        ...
        let count_subjects = subject_rows.length;
...
// - check if cluster_subjbase is selected subjbase (or no selected subjbase)
    ...
    //console.log("    allowed_subjectname_nl_arr", allowed_subjectname_nl_arr) ;
        ...
        if (allowed_subjectname_nl_arr.length) {
            if (allowed_subjectname_nl_arr.length === count_subjects){
                subjectname_nl_txt = loc.All_subjects;
            } else {
                allowed_subjectname_nl_arr.sort();
                subjectname_nl_txt = allowed_subjectname_nl_arr.join(", ");
                title_txt = allowed_subjectname_nl_arr.join("\n");
            };
        } else {
            subjectname_nl_txt = loc.All_subjects;
        };

        el_MAG_subject.innerText = subjectname_nl_txt;
        el_MAG_subject.title = title_txt;
    };  // MAG_set_subject_txt
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if ("grade_note_icon_rows" in response) {
                    ...
                    RefreshDataRows("grades", response.grade_note_icon_rows, grade_dicts, true); // true = is_update
                }
...
//=========  RefreshDataRowsAfterUploadFromExcel  ================ PR2021-07-20
    function RefreshDataRowsAfterUploadFromExcel(response) {
        ...
        if(!is_test && response && "updated_grade_rows" in response) {
            //RefreshDataRows("grade", response.updated_grade_rows, grade_dicts, true)  // true = update
...
//=========  RefreshDataRows  ================ PR2020-08-16 PR2020-09-30, PR2021-05-01 PR2021-09-20 PR2022-03-03 PR2023-04-03
...
// ++++ deleted ++++
    ...
    // ---  create list of updated fields
        ...
        // ---  if value has changed: add field to updated_columns before updating data_dict
                        ...
                        // it works when you get mapped_key after value comaprison PR2023-04-08
...
//========= ResetFilterRows  ====================================
    function ResetFilterRows() {  // PR2019-10-26 PR2020-06-20 PR2022-01-26 PR2023-04-09
...
//=========  MSSSS_subject_response  ================ PR2023-03-30 PR2024-03-28
...
//=========  MSSSS_cluster_response  ================ PR2023-03-30 PR2024-05-02
...
///////////////////////////////////////
// +++++++++ MOD SELECT EXAM  ++++++++++++++++ PR2022-06-22 PR2024-06-30
    ...
    console.log("data_dict", data_dict);
    console.log("    data_dict.exam_published_id", data_dict.exam_published_id);
    console.log("    data_dict.ce_exam_published_id", data_dict.ce_exam_published_id);
    console.log("    data_dict.ce_published_id", data_dict.ce_published_id);

                if (!!data_dict.ce_published_id){
    // exit and give message when the Ex2A score is submitted by the school
                    b_show_mod_message_html(["<p class='p-2 border_bg_invalid'>",
                        loc.approve_err_list.This_score_is_submitted, "<br>", loc.grade_err_list.You_cannot_change_exam,
                    "</p>"].join(""));

                } else if (!!data_dict.ce_auth1by_id || !!data_dict.ce_auth2by_id || !!data_dict.ce_auth3by_id || !!data_dict.ce_auth4by_id){
    // exit and give message when the Ex2A score is appproved by one or more auth
                   b_show_mod_message_html(["<p class='p-2 border_bg_invalid'>",
                        loc.approve_err_list.This_score_is_approved, "<br>", loc.grade_err_list.You_cannot_change_exam,
                    "</p>"].join(""));

                } else if (!!data_dict.ce_exam_published_id){
    // exit and give message when Wolf-exam is submitted
                   b_show_mod_message_html(["<p class='p-2 border_bg_invalid'>",
                        loc.grade_err_list.Wolf_scores_submitted, "<br>",
                        loc.grade_err_list.You_cannot_change_exam,  "<br>",
                        loc.grade_err_list.Wolf_remove_submitted1,"<br>",
                        loc.grade_err_list.Wolf_remove_submitted2,
                    "</p>"].join(""));

                } else if (!!data_dict.ce_exam_auth1by_id || !!data_dict.ce_exam_auth2by_id || !!data_dict.ce_exam_auth3by_id){
    // exit and give message when Wolf-exam is approved
                   b_show_mod_message_html(["<p class='p-2 border_bg_invalid'>",
                        loc.grade_err_list.Wolf_score_approved, "<br>", loc.grade_err_list.Wolf_remove_approved,
                    "</p>"].join(""));
    ...
    //console.log( "mod_MSELEX_dict.exam_pk", mod_MSELEX_dict.exam_pk);
        ...
        // ---  fill select table
                    ...
                    // show message when wolf scores are entered.
                    //This is the case whem ce_exam_result has value and a pipe '|' is in the value
                    // This has entered scores:    "37;39#1|1;2|3;a#2#3#4#5#6"
                    // This has no entered scores: "39;39#1#2#3#4#5#6"

                    const has_wolf_scores = (!!data_dict.ce_exam_result && data_dict.ce_exam_result.includes("|"));
                    if (has_wolf_scores) {
                        el_MSELEX_info_container.innerHTML =
                           ["<p class='p-2 border_bg_warning'><b>", loc.grade_err_list.ATTENTION, "</b>: ",
                            loc.grade_err_list.Wolf_scores_entered, "<br>",
                            loc.grade_err_list.Wolf_scores_willbe_deleted, "</p>"].join("");
                    }
                    add_or_remove_class(el_MSELEX_info_container, cls_hide, !has_wolf_scores )
                    add_or_remove_class(el_MSELEX_msg_container, cls_hide, true);
