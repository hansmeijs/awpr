...
# - get rest of keys
        for key, request_dict in request_item_setting.items():
            ...
            if key not in skip_keys:
                has_changed = False
                new_page_dict = {}
                saved_setting_dict = acc_mod.Usersetting.get_jsonsetting(key, req_user)
...
# get page settings - keys starting with 'page_'
                ...
                if has_changed:
                    acc_mod.Usersetting.set_jsonsetting(key, new_page_dict, req_user)
    #logger.debug('setting_dict: ' + str(setting_dict))

    # - save settings when they have changed
    if selected_dict_has_changed:
        # logger.debug('>>>>>>> save selected_dict: ' + str(selected_dict))
        acc_mod.Usersetting.set_jsonsetting(c.KEY_SELECTED_PK, selected_dict, req_user)
...
# - get role from req_user, put them in setting_dict PR2020-12-14
    setting_dict = {'user_lang': user_lang, 'requsr_pk': req_user.pk, 'requsr_name': req_user.last_name}
